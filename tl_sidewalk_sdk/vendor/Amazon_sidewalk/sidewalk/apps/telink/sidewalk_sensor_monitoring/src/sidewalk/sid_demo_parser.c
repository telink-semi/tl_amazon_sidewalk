...
void sid_demo_app_msg_serialize(struct sid_parse_state *const state, struct sid_demo_msg_desc *msg_desc, struct sid_demo_msg *msg)
{
    uint8_t value = 0;
    TL_LOG_E("Serialize: palylaod len:%d", msg->payload_size);
    log_buffer(msg->payload, msg->payload_size);

    set_bit_to_field(&value, (uint8_t)msg_desc->status_hdr_ind, SID_DEMO_APP_MSG_DESC_STATUS_HDR_IND_MASK, SID_DEMO_APP_MSG_DESC_STATUS_HDR_IND_OFFSET);
    set_bit_to_field(&value, (uint8_t)msg_desc->opc, SID_DEMP_APP_MSG_DESC_OP_CODE_MASK, SID_DEMO_APP_MSG_DESC_OP_CODE_OFFSET);
    set_bit_to_field(&value, (uint8_t)msg_desc->cmd_class, SID_DEMO_APP_MSG_DESC_CLASS_MASK, SID_DEMO_APP_MSG_DESC_CLASS_OFFSET);
    set_bit_to_field(&value, (uint8_t)msg_desc->cmd_id, SID_DEMP_APP_MSG_DESC_CMD_ID_MASK, SID_DEMO_APP_MSG_DESC_CMD_ID_OFFSET);
...
void sid_demo_app_msg_deserialize(struct sid_parse_state *const state, struct sid_demo_msg_desc *msg_desc, struct sid_demo_msg *msg)
{
    uint8_t value = 0;
    sid_read_entry_uint8(state, &value);
    get_bit_from_field(&value, (uint8_t *)&msg_desc->status_hdr_ind, SID_DEMO_APP_MSG_DESC_STATUS_HDR_IND_MASK, SID_DEMO_APP_MSG_DESC_STATUS_HDR_IND_OFFSET);
    get_bit_from_field(&value, (uint8_t *)&msg_desc->opc, SID_DEMP_APP_MSG_DESC_OP_CODE_MASK, SID_DEMO_APP_MSG_DESC_OP_CODE_OFFSET);
    get_bit_from_field(&value, &msg_desc->cmd_class, SID_DEMO_APP_MSG_DESC_CLASS_MASK, SID_DEMO_APP_MSG_DESC_CLASS_OFFSET);
    get_bit_from_field(&value, &msg_desc->cmd_id, SID_DEMP_APP_MSG_DESC_CMD_ID_MASK, SID_DEMO_APP_MSG_DESC_CMD_ID_OFFSET);
    if (msg_desc->status_hdr_ind) {
...
void sid_demo_app_capability_discovery_notification_serialize(struct sid_parse_state *const state, struct sid_demo_capability_discovery *cap)
{
    struct tl_in_tlv tl = {};
    if (cap->num_buttons) {
...
void sid_demo_app_capability_discovery_notification_deserialize(struct sid_parse_state *const state, struct sid_demo_capability_discovery *cap)
{
    struct tl_in_tlv tl = {};
...
void sid_demo_app_action_req_serialize(struct sid_parse_state *const state, struct sid_demo_led_action_req *led_action_req)
{
    struct tl_in_tlv tl = {};
...
void sid_demo_app_action_req_deserialize(struct sid_parse_state *const state, struct sid_demo_led_action_req *led_action_req)
{
    struct tl_in_tlv tl = {};
    while (state->offset < state->buffer_len) {
        sid_read_tl_from_tlv(state, &tl);
        ...
        case SID_DEMO_TAG_LED_OFF_ACTION_REQ:
            led_action_req->action_req = (tl.tag == SID_DEMO_TAG_LED_ON_ACTION_REQ) ? (SID_DEMO_ACTION_LED_ON) : (SID_DEMO_ACTION_LED_OFF);
...
void sid_demo_app_action_resp_serialize(struct sid_parse_state *const state, struct sid_demo_action_resp *action_resp)
{
    ...
    if (action_resp->resp_type == SID_DEMO_ACTION_TYPE_LED) {
        ...
        if (action_resp->led_action_resp.action_resp > SID_DEMO_ACTION_LED_NO_ACTION && action_resp->led_action_resp.action_resp < SID_DEMO_ACTION_LED_LAST) {
            tl.tag = (action_resp->led_action_resp.action_resp == SID_DEMO_ACTION_LED_ON) ? (SID_DEMO_TAG_LED_ON_RESP) : (SID_DEMO_TAG_LED_OFF_RESP);
            if (action_resp->led_action_resp.num_leds == 0xFF) {
                ...
                sid_write_entry_tlv_nbytes(state, &tl, action_resp->led_action_resp.led_id_arr, action_resp->led_action_resp.num_leds);
            }
        } else {
            goto err;
        }
    ...
    if (action_resp->resp_type == SID_DEMO_ACTION_TYPE_BUTTON) {
        ...
        if (action_resp->button_action_resp.num_buttons == 0xFF) {
            ...
            sid_write_entry_tlv_nbytes(state, &tl, action_resp->button_action_resp.button_id_arr, action_resp->button_action_resp.num_buttons);
        }
    }

    if (action_resp->resp_type == SID_DEMO_ACTION_TYPE_LINK_TYPE_CHANGE) {
        uint8_t err_code = 0; //OK
        tl.tag = SID_DEMO_TAG_LINK_TYPE_SET_RESP;
        sid_write_entry_tlv_uint8(state, &tl, err_code);
...
err:
    state->ret_code = SID_ERROR_INVALID_ARGS;
    TL_LOG_E("sid_demo_app_action_resp_serialize: SID_ERROR_INVALID_ARGS");
}
...
void sid_demo_app_action_resp_deserialize(struct sid_parse_state *const state, struct sid_demo_action_resp *action_resp)
{
    ...
    while (state->offset < state->buffer_len) {
        ...
        case SID_DEMO_TAG_LED_OFF_RESP:
            action_resp->led_action_resp.action_resp = (tl.tag == SID_DEMO_TAG_LED_ON_RESP) ? (SID_DEMO_ACTION_LED_ON) : (SID_DEMO_ACTION_LED_OFF);
        ...
        case SID_DEMO_TAG_BUTTON_PRESSED_RESP:
            ...
            sid_read_entry_nbytes(state, action_resp->button_action_resp.button_id_arr, action_resp->button_action_resp.num_buttons);
...
void sid_demo_app_action_notification_serialize(struct sid_parse_state *const state, struct sid_demo_action_notification *action_notify)
{
    struct tl_in_tlv tl = {};
    ...
    if (action_notify->button_action_notify.action_resp == SID_DEMO_ACTION_BUTTON_PRESSED) {
        ...
        if (action_notify->button_action_notify.num_buttons == 0xFF) {
            ...
            sid_write_entry_tlv_nbytes(state, &tl, action_notify->button_action_notify.button_id_arr, action_notify->button_action_notify.num_buttons);
        }
    }

    if ((action_notify->temp_sensor > SID_DEMO_TEMPERATURE_SENSOR_NOT_SUPPORTED) && (action_notify->temp_sensor < SID_DEMO_TEMPERATURE_SENSOR_LAST)) {
    ...
    if(action_notify->IMU_data_used == 0x01) {
        tl.tag = SID_DEMO_TAG_IMU_DATA;
        tl.len = IMU_DATA_SIZE;
        sid_write_entry_tlv_nbytes(state, &tl, action_notify->IMU_data, IMU_DATA_SIZE);
    }

    if(action_notify->number_of_active_LEDs > 0) {
        tl.tag = SID_DEMO_TAG_LED_ON_RESP;
        tl.len = action_notify->number_of_active_LEDs;
        sid_write_entry_tlv_nbytes(state, &tl, action_notify->LEDs_active, action_notify->number_of_active_LEDs);
    }
...
void sid_demo_app_action_notification_deserialize(struct sid_parse_state *const state, struct sid_demo_action_notification *action_notify)
{
    struct tl_in_tlv tl = {};
    ...
    while (state->offset < state->buffer_len) {
        ...
        case SID_DEMO_TAG_BUTTON_PRESS_ACTION_NOTIFY:
            ...
            sid_read_entry_nbytes(state, action_notify->button_action_notify.button_id_arr, action_notify->button_action_notify.num_buttons);
