                                                              ...
                                                              struct sid_demo_capability_discovery *cap)
{
    ...
    if (cap->temp_sensor > SID_DEMO_TEMPERATURE_SENSOR_NOT_SUPPORTED
        && cap->temp_sensor < SID_DEMO_TEMPERATURE_SENSOR_LAST) {
    ...
    if (cap->ota_support) {
        tl.tag = SID_DEMO_TAG_OTA_SUPPORTED;
        tl.len = sizeof(cap->ota_support);
        sid_write_entry_tlv_uint8(state, &tl, cap->ota_support);
    }

    if (cap->major || cap->minor || cap->patch || cap->build) {
        uint8_t version[10] = {};
        struct sid_parse_state fw_state;
        sid_parse_state_init(&fw_state, version, sizeof(version));
        sid_write_entry_uint16(&fw_state, cap->major);
        sid_write_entry_uint16(&fw_state, cap->minor);
        sid_write_entry_uint16(&fw_state, cap->patch);
        sid_write_entry_uint16(&fw_state, cap->build);

        tl.tag = SID_DEMO_TAG_OTA_FIRMWARE_VERSION;
        tl.len = fw_state.offset;
        sid_write_entry_tlv_nbytes(state, &tl, version, fw_state.offset);
    }
    ...
    while (state->offset < state->buffer_len) {
        ...
        switch (tl.tag) {
            ...
            case SID_DEMO_TAG_LINK_TYPE:
                ...
                break;
            case SID_DEMO_TAG_OTA_SUPPORTED:
                sid_read_entry_uint8(state, &cap->ota_support);
                break;
            case SID_DEMO_TAG_OTA_FIRMWARE_VERSION: {
                sid_read_entry_uint16(state, &cap->major);
                sid_read_entry_uint16(state, &cap->minor);
                sid_read_entry_uint16(state, &cap->patch);
                sid_read_entry_uint16(state, &cap->build);
            } break;
            default:
                break;
        }
    ...
    while (state->offset < state->buffer_len) {
        ...
        switch (tl.tag) {
            ...
            case SID_DEMO_TAG_LED_OFF_ACTION_REQ:
                led_action_req->action_req =
                    (tl.tag == SID_DEMO_TAG_LED_ON_ACTION_REQ) ? (SID_DEMO_ACTION_LED_ON) : (SID_DEMO_ACTION_LED_OFF);
...
void sid_demo_app_action_resp_serialize(struct sid_parse_state *const state, struct sid_demo_action_resp *action_resp)
{
    ...
    if (action_resp->resp_type == SID_DEMO_ACTION_TYPE_LED) {
        ...
        if (action_resp->led_action_resp.action_resp > SID_DEMO_ACTION_LED_NO_ACTION
            ...
            tl.tag = (action_resp->led_action_resp.action_resp == SID_DEMO_ACTION_LED_ON) ? (SID_DEMO_TAG_LED_ON_RESP)
                                                                                          : (SID_DEMO_TAG_LED_OFF_RESP);
    ...
    while (state->offset < state->buffer_len) {
        ...
        switch (tl.tag) {
            ...
            case SID_DEMO_TAG_LED_OFF_RESP:
                action_resp->led_action_resp.action_resp =
                    (tl.tag == SID_DEMO_TAG_LED_ON_RESP) ? (SID_DEMO_ACTION_LED_ON) : (SID_DEMO_ACTION_LED_OFF);
                action_resp->led_action_resp.num_leds = tl.len;
                sid_read_entry_nbytes(state, action_resp->led_action_resp.led_id_arr,
                                      action_resp->led_action_resp.num_leds);
                                                ...
                                                struct sid_demo_action_notification *action_notify)
{
    ...
    if (action_notify->trigger_ota) {
        tl.tag = SID_DEMO_TAG_OTA_TRIGGER_NOTIFY;
        sid_write_entry_tlv_uint8(state, &tl, action_notify->trigger_ota);
    }

    bool include_file_id = false;

    if (action_notify->ota_stats.is_valid) {
        struct sid_parse_state ota_stats;
        uint8_t stats_data[sizeof(action_notify->ota_stats)] = {};
        sid_parse_state_init(&ota_stats, stats_data, sizeof(stats_data));
        sid_write_entry_uint8(&ota_stats, action_notify->ota_stats.percent);
        sid_write_entry_uint32(&ota_stats, action_notify->ota_stats.completed_file_size);
        sid_write_entry_uint32(&ota_stats, action_notify->ota_stats.total_file_size);
        tl.tag = SID_DEMO_TAG_OTA_STATS;
        tl.len = ota_stats.offset;
        sid_write_entry_tlv_nbytes(state, &tl, stats_data, ota_stats.offset);
        include_file_id = true;
    }

    if (action_notify->ota_status != SID_DEMO_OTA_COMPLETION_STATUS_INITIAL) {
        tl.tag = SID_DEMO_TAG_OTA_COMPLETION_STATUS;
        tl.len = 1;
        sid_write_entry_tlv_uint8(state, &tl, action_notify->ota_status);
        include_file_id = true;
    }

    if (include_file_id) {
        tl.tag = SID_DEMO_TAG_OTA_FILE_ID;
        tl.len = sizeof(action_notify->file_id);
        sid_write_entry_tlv_uint32(state, &tl, action_notify->file_id);
    }

    if (action_notify->gps_time_in_seconds) {
        tl.tag = SID_DEMO_TAG_CURRENT_GPS_TIME_IN_SECONDS;
    ...
    while (state->offset < state->buffer_len) {
        ...
        switch (tl.tag) {
            ...
            case SID_DEMO_TAG_LINK_TYPE:
                ...
                break;
            case SID_DEMO_TAG_OTA_TRIGGER_NOTIFY:
                sid_read_entry_uint8(state, &action_notify->trigger_ota);
                break;
            case SID_DEMO_TAG_OTA_COMPLETION_STATUS:
                sid_read_entry_uint8(state, (uint8_t *)&action_notify->ota_status);
                break;
            case SID_DEMO_TAG_OTA_STATS: {
                uint8_t stats_data[sizeof(action_notify->ota_stats)] = {};
                sid_read_entry_nbytes(state, stats_data, tl.len < sizeof(stats_data) ? tl.len : sizeof(stats_data));
                struct sid_parse_state ota_stats;
                sid_parse_state_init(&ota_stats, stats_data, sizeof(stats_data));
                sid_read_entry_uint8(&ota_stats, &action_notify->ota_stats.percent);
                sid_read_entry_uint32(&ota_stats, &action_notify->ota_stats.completed_file_size);
                sid_read_entry_uint32(&ota_stats, &action_notify->ota_stats.total_file_size);
                action_notify->ota_stats.is_valid = true;
            } break;
            case SID_DEMO_TAG_OTA_FILE_ID:
                sid_read_entry_uint32(state, &action_notify->file_id);
                break;
            default:
                break;
        }
...
{
    sid_read_entry_uint32(state, &sdk_version->major);
    sid_read_entry_uint32(state, &sdk_version->minor);
    sid_read_entry_uint32(state, &sdk_version->patch);
    sid_read_entry_uint32(state, &sdk_version->build);
}

void sid_demo_sdk_version_serialize(struct sid_parse_state *const state, struct sid_demo_sdk_version *sdk_version)
{
    sid_write_entry_uint32(state, sdk_version->major);
    sid_write_entry_uint32(state, sdk_version->minor);
    sid_write_entry_uint32(state, sdk_version->patch);
    sid_write_entry_uint32(state, sdk_version->build);
}
