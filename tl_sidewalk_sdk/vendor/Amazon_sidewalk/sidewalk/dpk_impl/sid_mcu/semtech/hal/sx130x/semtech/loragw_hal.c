...
/* --- PRIVATE MACROS ------------------------------------------------------- */
    ...
    #define DEBUG_ARRAY(a,b,c)            for(a=0; a<b; ++a) SID_HAL_LOG_INFO("%x.",c[a]);SID_HAL_LOG_INFO("end\n")
    ...
    .rf_chain_cfg = {
        {
            ...
            .freq_hz = 0,
            .rssi_offset = 0.0,
            .rssi_tcomp = { .coeff_a = 0.0, .coeff_b = 0.0, .coeff_c = 20.41, .coeff_d = 2162.56, .coeff_e = 0.0 },
...
const lgw_context_t *lgw_get_context(void)
{
    return &lgw_context;
}
...
static int remove_pkt(struct lgw_pkt_rx_s * p, uint8_t * nb_pkt, uint8_t pkt_index) {
    ...
    if (pkt_index > ((*nb_pkt) - 1)) {
        SID_HAL_LOG_ERROR("ERROR: failed to remove packet index %u\n", pkt_index);
        return -1;
    }
        ...
        for (k = (j+1); k < cpt; k++) {
            ...
            */
            if (is_same_pkt( &p[j], &p[k])) {
                ...
                if (x != 0) {
                    SID_HAL_LOG_ERROR("ERROR: failed to remove packet from array (%d)\n", x);
                }
                dup_restart = true;
                break;
            }
...
#if 1// not support qsort_r() , replace by qsort()
    qsort(p, cpt, sizeof(p[0]), (__compar_fn_t)&counter_qsort_swap);
#else
    qsort_r(p, cpt, sizeof(p[0]), compare_pkt_tmst,
            (int(*)(void *, const void *, const void *))&counter_qsort_swap);
#endif
...
int lgw_board_setconf(struct lgw_conf_board_s * conf) {
    ...
    if (CONTEXT_STARTED == true) {
        SID_HAL_LOG_ERROR("ERROR: CONCENTRATOR IS RUNNING, STOP IT BEFORE TOUCHING CONFIGURATION\n");
        return LGW_HAL_ERROR;
    }

    /* Check input parameters */
    if (conf->com_type != LGW_COM_SPI) {
        SID_HAL_LOG_ERROR("ERROR: WRONG COM TYPE\n");
        return LGW_HAL_ERROR;
    }
...
int lgw_rxrf_setconf(uint8_t rf_chain, struct lgw_conf_rxrf_s * conf) {
    ...
    if (conf->enable == false) {
        /* nothing to do */
        SID_HAL_LOG_INFO("Note: rf_chain %d disabled\n", rf_chain);
        return LGW_HAL_SUCCESS;
    }

    /* check input range (segfault prevention) */
    if (rf_chain >= LGW_RF_CHAIN_NB) {
        SID_HAL_LOG_ERROR("ERROR: NOT A VALID RF_CHAIN NUMBER\n");
        return LGW_HAL_ERROR;
    }
    ...
    if ((conf->type != LGW_RADIO_TYPE_SX1255) && (conf->type != LGW_RADIO_TYPE_SX1257) && (conf->type != LGW_RADIO_TYPE_SX1250)) {
        SID_HAL_LOG_ERROR("ERROR: NOT A VALID RADIO TYPE (%d)\n", conf->type);
        return LGW_HAL_ERROR;
    }

    /* check if the radio central frequency is valid */
    if ((conf->freq_hz < LGW_RF_RX_FREQ_MIN) || (conf->freq_hz > LGW_RF_RX_FREQ_MAX)) {
        SID_HAL_LOG_ERROR("ERROR: NOT A VALID RADIO CENTER FREQUENCY, PLEASE CHECK IF IT HAS BEEN GIVEN IN HZ (%u)\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
    ...
    DEBUG_PRINTF("Note: rf_chain %d configuration; en:%d freq:%d radio_type:%d tx_enable:%d single_input_mode:%d\n",
                     rf_chain,
...
int lgw_rxif_setconf(uint8_t if_chain, struct lgw_conf_rxif_s * conf) {
    ...
    if (if_chain >= LGW_IF_CHAIN_NB) {
        SID_HAL_LOG_ERROR("ERROR: %d NOT A VALID IF_CHAIN NUMBER\n", if_chain);
        return LGW_HAL_ERROR;
    }
    ...
    if (sx1302_get_ifmod_config(if_chain) == IF_UNDEFINED) {
        SID_HAL_LOG_ERROR("ERROR: IF CHAIN %d NOT CONFIGURABLE\n", if_chain);
    }
    if (conf->rf_chain >= LGW_RF_CHAIN_NB) {
        SID_HAL_LOG_ERROR("ERROR: INVALID RF_CHAIN TO ASSOCIATE WITH A LORA_STD IF CHAIN\n");
        return LGW_HAL_ERROR;
    }
...
#if 1//NOTE: we don't support negative IF_FREQ config to prevent boundary checking error.
    if ((conf->freq_hz + ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) > (int32_t)rf_rx_bandwidth) {
        SID_HAL_LOG_ERROR("ERROR: IF FREQUENCY %d TOO HIGH\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    } else if (conf->freq_hz < 0) {
        SID_HAL_LOG_ERROR("ERROR: IF FREQUENCY %d NOT SUPPORT NEGATIVE\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
#else
    if ((conf->freq_hz + ((bw_hz==-1)?LGW_REF_BW:bw_hz)/2) > ((int32_t)rf_rx_bandwidth/2)) {
        ...
        SID_HAL_LOG_ERROR("ERROR: IF FREQUENCY %d TOO LOW\n", conf->freq_hz);
        return LGW_HAL_ERROR;
    }
#endif
    ...
    switch (sx1302_get_ifmod_config(if_chain)) {
        case IF_LORA_STD:
            ...
            if (!IS_LORA_BW(conf->bandwidth)) {
                SID_HAL_LOG_ERROR("ERROR: BANDWIDTH NOT SUPPORTED BY LORA_STD IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if (!IS_LORA_DR(conf->datarate)) {
                SID_HAL_LOG_ERROR("ERROR: DATARATE NOT SUPPORTED BY LORA_STD IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
        ...
        case IF_LORA_MULTI:
            ...
            if (conf->bandwidth != BW_125KHZ) {
                SID_HAL_LOG_ERROR("ERROR: BANDWIDTH NOT SUPPORTED BY LORA_MULTI IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if (!IS_LORA_DR(conf->datarate)) {
                SID_HAL_LOG_ERROR("ERROR: DATARATE(S) NOT SUPPORTED BY LORA_MULTI IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            /* set internal configuration  */
            CONTEXT_IF_CHAIN[if_chain].enable = conf->enable;
            CONTEXT_IF_CHAIN[if_chain].rf_chain = conf->rf_chain;
            CONTEXT_IF_CHAIN[if_chain].freq_hz = conf->freq_hz;
            CONTEXT_IF_CHAIN[if_chain].implicit_crc_en = conf->implicit_crc_en;
        ...
        case IF_FSK_STD:
            ...
            if(!IS_FSK_BW(conf->bandwidth)) {
                SID_HAL_LOG_ERROR("ERROR: BANDWIDTH NOT SUPPORTED BY FSK IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            if(!IS_FSK_DR(conf->datarate)) {
                SID_HAL_LOG_ERROR("ERROR: DATARATE NOT SUPPORTED BY FSK IF CHAIN\n");
                return LGW_HAL_ERROR;
            }
            /* set internal configuration  */
            CONTEXT_IF_CHAIN[if_chain].enable = conf->enable;
        ...
        default:
            SID_HAL_LOG_ERROR("ERROR: IF CHAIN %d TYPE NOT SUPPORTED\n", if_chain);
            return LGW_HAL_ERROR;
    }
...
int lgw_txgain_setconf(uint8_t rf_chain, struct lgw_tx_gain_lut_s * conf) {
    ...
    if ((conf->size < 1) || (conf->size > TX_GAIN_LUT_SIZE_MAX)) {
        SID_HAL_LOG_ERROR("ERROR: TX gain LUT must have at least one entry and  maximum %d entries\n", TX_GAIN_LUT_SIZE_MAX);
        return LGW_HAL_ERROR;
    }
    ...
    for (i = 0; i < CONTEXT_TX_GAIN_LUT[rf_chain].size; i++) {
        ...
        if (conf->lut[i].dig_gain > 3) {
            SID_HAL_LOG_ERROR("ERROR: TX gain LUT: SX1302 digital gain must be between 0 and 3\n");
            return LGW_HAL_ERROR;
        }
        if (conf->lut[i].dac_gain > 3) {
            SID_HAL_LOG_ERROR("ERROR: TX gain LUT: SX1257 DAC gains must not exceed 3\n");
            return LGW_HAL_ERROR;
        }
        if ((conf->lut[i].mix_gain < 5) || (conf->lut[i].mix_gain > 15)) {
            SID_HAL_LOG_ERROR("ERROR: TX gain LUT: SX1257 mixer gain must be betwen [5..15]\n");
            return LGW_HAL_ERROR;
        }
        if (conf->lut[i].pa_gain > 3) {
            SID_HAL_LOG_ERROR("ERROR: TX gain LUT: External PA gain must not exceed 3\n");
            return LGW_HAL_ERROR;
        }

        if ((conf->lut[i].pwr_idx >= 18 && conf->lut[i].pwr_idx <= 22) ||
            (conf->lut[i].pwr_idx >= 50 && conf->lut[i].pwr_idx <= 54)) {
            conf->lut[i].pa_gain = 0; // force turn off ext PA to protect it.
        }
...
int lgw_start(void) {
    ...
    if (CONTEXT_STARTED == true) {
        SID_HAL_LOG_INFO("Note: LoRa concentrator already started, restarting it now\n");
    }

    err = lgw_connect(CONTEXT_COM_TYPE, CONTEXT_COM_PATH);
    if (err == LGW_REG_ERROR) {
        SID_HAL_LOG_ERROR("ERROR: FAIL TO CONNECT BOARD\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to set all GPIOs to 0\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: radio calibration failed\n");
        return LGW_HAL_ERROR;
    }
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (CONTEXT_RF_CHAIN[i].enable == true) {
            ...
            if (err != LGW_REG_SUCCESS) {
                SID_HAL_LOG_ERROR("ERROR: failed to reset radio %d\n", i);
                return LGW_HAL_ERROR;
            }
            ...
            switch (CONTEXT_RF_CHAIN[i].type) {
                case LGW_RADIO_TYPE_SX1250:
                    ...
                    break;
                default:
                    SID_HAL_LOG_ERROR("ERROR: RADIO TYPE NOT SUPPORTED (RF_CHAIN %d)\n", i);
                    return LGW_HAL_ERROR;
            }
            if (err != LGW_REG_SUCCESS) {
                SID_HAL_LOG_ERROR("ERROR: failed to setup radio %d\n", i);
                return LGW_HAL_ERROR;
            }
            ...
            if (err != LGW_REG_SUCCESS) {
                SID_HAL_LOG_ERROR("ERROR: failed to set mode for radio %d\n", i);
                return LGW_HAL_ERROR;
            }
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to get clock from radio %u\n", CONTEXT_BOARD.clksrc);
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to release control over radios\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to initialize SX1302\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 PA/LNA LUT\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 radio frontend\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 channelizer\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 LoRa modem correlators\n");
        return LGW_HAL_ERROR;
    }
    err = sx1302_lora_modem_configure(CONTEXT_RF_CHAIN[0].freq_hz);
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 LoRa modems\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (CONTEXT_IF_CHAIN[8].enable == true) {
        ...
        if (err != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 LoRa Service modem correlators\n");
            return LGW_HAL_ERROR;
        }
        err = sx1302_lora_service_modem_configure(&(CONTEXT_LORA_SERVICE), CONTEXT_RF_CHAIN[0].freq_hz);
        if (err != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 LoRa Service modem\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (CONTEXT_IF_CHAIN[9].enable == true) {
        ...
        if (err != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 FSK modem\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 LoRa syncword\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to enable SX1302 modems\n");
        return LGW_HAL_ERROR;
    }
    ...
    switch (CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type) {
        case LGW_RADIO_TYPE_SX1250:
            ...
            if (err != LGW_REG_SUCCESS) {
                SID_HAL_LOG_ERROR("ERROR: failed to load AGC firmware for sx1250\n");
                return LGW_HAL_ERROR;
            }
            break;
        case LGW_RADIO_TYPE_SX1255:
        case LGW_RADIO_TYPE_SX1257: {
                SID_HAL_LOG_ERROR("ERROR: failed to load AGC firmware for sx125x\n");
                return LGW_HAL_ERROR;
            }
            break;
        default:
    ...
    err = sx1302_agc_start(FW_VERSION_AGC, CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type,
                           SX1302_AGC_RADIO_GAIN_AUTO, SX1302_AGC_RADIO_GAIN_AUTO,
                           CONTEXT_BOARD.full_duplex, CONTEXT_SX1261.lbt_conf.enable);
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to start AGC firmware\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to load ARB firmware\n");
        return LGW_HAL_ERROR;
    }
    err = sx1302_arb_start(FW_VERSION_ARB, &CONTEXT_FINE_TIMESTAMP);
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to start ARB firmware\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure SX1302 TX path\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to enable GPS on sx1302\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to set CONFIG_DONE GPIO\n");
        return LGW_HAL_ERROR;
    }
...
int lgw_stop(void) {
    ...
    if (CONTEXT_STARTED == false) {
        SID_HAL_LOG_INFO("Note: LoRa concentrator was not started...\n");
        return LGW_HAL_SUCCESS;
    }
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        ...
        if (x != LGW_HAL_SUCCESS) {
            SID_HAL_LOG_WARNING("WARNING: failed to get abort TX on chain %u\n", i);
            err = LGW_HAL_ERROR;
        }
    }
    ...
    x = lgw_disconnect();
    if (x != LGW_HAL_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to disconnect concentrator\n");
        err = LGW_HAL_ERROR;
    }
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    if (res != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to fetch packets from SX1302\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (nb_pkt_fetched > max_pkt) {
        DEBUG_PRINTF("WARNING: not enough space allocated, fetched %d packet(s), \
                     %d will be left in RX buffer\n", nb_pkt_fetched, (nb_pkt_fetched - max_pkt));
    }

    /* Apply RSSI temperature compensation */
    res = lgw_get_temperature(&current_temperature);
    if (res != LGW_HAL_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to get current temperature\n");
        return LGW_HAL_ERROR;
    }
    ...
    DEBUG_PRINTF("INFO: nb pkt found:%u\n", nb_pkt_found);
    ...
    if ((nb_pkt_found > 0) && (CONTEXT_FINE_TIMESTAMP.enable == true)) {
        ...
        if (res != 0) {
            SID_HAL_LOG_WARNING("WARNING: failed to remove duplicated packets\n");
        }
...
int lgw_send(struct lgw_pkt_tx_s * pkt_data) {
    ...
    /* check if the concentrator is running */
    if (CONTEXT_STARTED == false) {
        SID_HAL_LOG_ERROR("ERROR: CONCENTRATOR IS NOT RUNNING, START IT BEFORE SENDING\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (pkt_data->rf_chain >= LGW_RF_CHAIN_NB) {
        SID_HAL_LOG_ERROR("ERROR: INVALID RF_CHAIN TO SEND PACKETS\n");
        return LGW_HAL_ERROR;
    }

    /* check input variables */
    if (CONTEXT_RF_CHAIN[pkt_data->rf_chain].tx_enable == false) {
        SID_HAL_LOG_ERROR("ERROR: SELECTED RF_CHAIN IS DISABLED FOR TX ON SELECTED BOARD\n");
        return LGW_HAL_ERROR;
    }
    if (CONTEXT_RF_CHAIN[pkt_data->rf_chain].enable == false) {
        SID_HAL_LOG_ERROR("ERROR: SELECTED RF_CHAIN IS DISABLED\n");
        return LGW_HAL_ERROR;
    }
    if (!IS_TX_MODE(pkt_data->tx_mode)) {
        SID_HAL_LOG_ERROR("ERROR: TX_MODE NOT SUPPORTED\n");
        return LGW_HAL_ERROR;
    }
    if (pkt_data->modulation == MOD_LORA) {
        if (!IS_LORA_BW(pkt_data->bandwidth)) {
            SID_HAL_LOG_ERROR("ERROR: BANDWIDTH NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (!IS_LORA_DR(pkt_data->datarate)) {
            SID_HAL_LOG_ERROR("ERROR: DATARATE NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (!IS_LORA_CR(pkt_data->coderate)) {
            SID_HAL_LOG_ERROR("ERROR: CODERATE NOT SUPPORTED BY LORA TX\n");
            return LGW_HAL_ERROR;
        }
        if (pkt_data->size > 255) {
            SID_HAL_LOG_ERROR("ERROR: PAYLOAD LENGTH TOO BIG FOR LORA TX\n");
            return LGW_HAL_ERROR;
        }
    } else if (pkt_data->modulation == MOD_FSK) {
        if((pkt_data->f_dev < 1) || (pkt_data->f_dev > 200)) {
            SID_HAL_LOG_ERROR("ERROR: TX FREQUENCY DEVIATION OUT OF ACCEPTABLE RANGE\n");
            return LGW_HAL_ERROR;
        }
        ...
        if (pkt_data->size > 255) {
            SID_HAL_LOG_ERROR("ERROR: PAYLOAD LENGTH TOO BIG FOR FSK TX\n");
            return LGW_HAL_ERROR;
        }
    } else if (pkt_data->modulation == MOD_CW) {
        /* do nothing */
    } else {
        SID_HAL_LOG_ERROR("ERROR: INVALID TX MODULATION\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: %s: Failed to send packet\n", __FUNCTION__);
...
int lgw_status(uint8_t rf_chain, uint8_t select, uint8_t *code) {
    ...
    if (select == TX_STATUS) {
        ...
        SID_HAL_LOG_ERROR("ERROR: SELECTION INVALID, NO STATUS TO RETURN\n");
        return LGW_HAL_ERROR;
    }
...
uint32_t lgw_time_on_air(const struct lgw_pkt_tx_s *packet) {
    ...
    if (packet == NULL) {
        SID_HAL_LOG_ERROR("ERROR: Failed to compute time on air, wrong parameter\n");
        return 0;
    }
    ...
    if (packet->modulation == MOD_LORA) {
        ...
        */
        ...
        toa_ms = 0;
        SID_HAL_LOG_ERROR("ERROR: Cannot compute time on air for this packet, unsupported modulation (0x%02X)\n", packet->modulation);
    }
