...
int32_t legacy_timestamp_correction(uint8_t bandwidth, uint8_t sf, uint8_t cr, bool crc_en, uint8_t payload_length, sx1302_rx_dft_peak_mode_t dft_peak_mode) {
    ...
    switch (bandwidth)
    {
        ...
        case BW_500KHZ:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: UNEXPECTED VALUE %d IN SWITCH STATEMENT - %s\n", bandwidth, __FUNCTION__);
            return 0;
    }
    ...
    if (total_delay > INT32_MAX) {
        SID_HAL_LOG_ERROR("ERROR: overflow error for timestamp correction (SHOULD NOT HAPPEN)\n");
        SID_HAL_LOG_INFO("=> filtering_delay %" PRIu64 "\n", filtering_delay);
        SID_HAL_LOG_INFO("=> fft_delay_state3 %" PRIu64 "\n", fft_delay_state3);
        SID_HAL_LOG_INFO("=> fft_delay %" PRIu64 "\n", fft_delay);
        SID_HAL_LOG_INFO("=> demap_delay %" PRIu64 "\n", demap_delay);
        SID_HAL_LOG_INFO("=> decode_delay %" PRIu64 "\n", decode_delay);
        SID_HAL_LOG_INFO("=> total_delay %" PRIu64 "\n", total_delay);
        SID_PAL_ASSERT(0);
    }
...
int32_t precision_timestamp_correction(uint8_t bandwidth, uint8_t datarate, uint8_t coderate, bool crc_en, uint8_t payload_length) {
    ...
    if (lora_packet_time_on_air(bandwidth, datarate, coderate, 0, false, !crc_en, payload_length, NULL, &nb_symbols_payload, &t_symbol_us) == 0) {
        SID_HAL_LOG_ERROR("ERROR: failed to compute packet time on air - %s\n", __FUNCTION__);
        return 0;
    }
...
int timestamp_counter_get(timestamp_counter_t * self, uint32_t * inst, uint32_t * pps) {
    ...
    */
    ...
    if (x != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to get timestamp counter value\n");
        return -1;
    }
    ...
    if (x != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to get timestamp counter MSB value\n");
        return -1;
    }
...
int32_t timestamp_counter_correction(lgw_context_t * context, uint8_t bandwidth, uint8_t datarate, uint8_t coderate, bool crc_en, uint8_t payload_length, sx1302_rx_dft_peak_mode_t dft_peak_mode) {
    ...
    if (IS_LORA_DR(datarate) == false) {
        SID_HAL_LOG_ERROR("ERROR: wrong datarate (%u) - %s\n", datarate, __FUNCTION__);
        return 0;
    }
    if (IS_LORA_BW(bandwidth) == false) {
        SID_HAL_LOG_ERROR("ERROR: wrong bandwidth (%u) - %s\n", bandwidth, __FUNCTION__);
        return 0;
    }
    if (IS_LORA_CR(coderate) == false) {
        SID_HAL_LOG_ERROR("ERROR: wrong coding rate (%u) - %s\n", coderate, __FUNCTION__);
        return 0;
    }
...
int precise_timestamp_calculate(uint8_t ts_metrics_nb, const int8_t * ts_metrics, uint32_t timestamp_cnt, uint8_t sf, int32_t if_freq_hz, double pkt_freq_error, uint32_t * result_ftime) {
    ...
    if (timestamp_pps_history.size < MAX_TIMESTAMP_PPS_HISTORY) {
        SID_HAL_LOG_INFO("INFO: Cannot compute ftime yet, PPS history is too short\n");
        return -1;
    }
    ...
    if ((timestamp_cnt - timestamp_pps_reg) > 32e6) {
        ...
        if (timestamp_pps_idx == timestamp_pps_history.size) {
            SID_HAL_LOG_ERROR("ERROR: failed to find the reference timestamp_pps, cannot compute ftime\n");
            return -1;
        }
    ...
    if ((xtal_correct > 1.2) || (xtal_correct < 0.8)) {
        SID_HAL_LOG_ERROR("ERROR: xtal_error is invalid (%.15lf)\n", xtal_correct);
        return -1;
    }
    ...
    *result_ftime = (uint32_t)pkt_ftime;
    if (*result_ftime > 1E9) {
        SID_HAL_LOG_ERROR("ERROR: fine timestamp is out of range (%lu)\n", *result_ftime);
        return -1;
    }
