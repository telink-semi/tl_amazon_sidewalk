...
int sx1250_reg_w(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain)
{
    ...
    if (rf_chain >= LGW_RF_CHAIN_NB) {
        SID_HAL_LOG_ERROR("ERROR: INVALID RF_CHAIN\n");
        return LGW_REG_ERROR;
    }

    com_stat = sx1250_com_w(lgw_com_type(), lgw_com_target(),
                            ((rf_chain == 0)? LGW_SPI_MUX_TARGET_RADIOA: LGW_SPI_MUX_TARGET_RADIOB),
                            op_code, data, size);

    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: COM ERROR DURING RADIO REGISTER WRITE\n");
        return LGW_REG_ERROR;
    } else {
...
int sx1250_reg_r(sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain)
{
    ...
    com_stat = sx1250_com_r(lgw_com_type(), lgw_com_target(),
                            ((rf_chain == 0)? LGW_SPI_MUX_TARGET_RADIOA: LGW_SPI_MUX_TARGET_RADIOB),
                            op_code, data, size);

    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: COM ERROR DURING RADIO REGISTER READ\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int sx1250_calibrate(uint8_t rf_chain, uint32_t freq_hz)
{
    ...
    if ((freq_hz > 430E6) && (freq_hz < 440E6)) {
        ...
        SID_HAL_LOG_ERROR("ERROR: failed to calibrate sx1250 radio, frequency range not supported (%lu)\n", freq_hz);
        return LGW_REG_ERROR;
    }
    ...
    if (TAKE_N_BITS_FROM(buff[2], 4, 1) != 0) {
        SID_HAL_LOG_ERROR("ERROR: sx1250 Image Calibration Error\n");
        return LGW_REG_ERROR;
    }
...
int sx1250_setup(uint8_t rf_chain, uint32_t freq_hz, bool single_input_mode)
{
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x02) {
        SID_HAL_LOG_ERROR("ERROR: Failed to set SX1250_%u in STANDBY_RC mode\n", rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if ((uint8_t)(TAKE_N_BITS_FROM(buff[0], 4, 3)) != 0x03) {
        SID_HAL_LOG_ERROR("ERROR: Failed to set SX1250_%u in STANDBY_XOSC mode\n", rf_chain);
        return LGW_REG_ERROR;
    }
    ...
    if (single_input_mode == true) {
        SID_HAL_LOG_INFO("INFO: Configuring SX1250_%u in single input mode\n", rf_chain);
        buff[0] = 0x08;
        buff[1] = 0xE2;
        buff[2] = 0x0D;
        err |= sx1250_reg_w(WRITE_REGISTER, buff, 3, rf_chain);
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to setup SX1250_%u radio\n", rf_chain);
        return LGW_REG_ERROR;
    }
