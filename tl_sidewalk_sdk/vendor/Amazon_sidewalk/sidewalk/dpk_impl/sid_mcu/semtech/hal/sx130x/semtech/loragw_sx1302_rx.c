...
/* --- PRIVATE MACROS ------------------------------------------------------- */
    ...
    #define DEBUG_PRINTF(fmt, args...)    SID_HAL_LOG_INFO(fmt, args)
...
int rx_buffer_fetch(rx_buffer_t * self) {
    ...
    if (self->buffer_size > 0) {
        ...
        if (res != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: Failed to read RX buffer, SPI error\n");
            return LGW_REG_ERROR;
        }

        /* print debug info */
    #if (DEBUG_SX1302 == 1)
        SID_HAL_LOG_INFO("RX_BUFFER: ");
        SID_HAL_LOG_HEXDUMP_INFO(self->buffer, self->buffer_size);
    #endif
        ...
        if (self->buffer_size < (SX1302_PKT_HEAD_METADATA + SX1302_PKT_TAIL_METADATA)) {
            SID_HAL_LOG_WARNING("WARNING: not enough data to have a complete packet, discard rx_buffer\n");
        ...
        while (idx <= (self->buffer_size - 2)) {
            if ((self->buffer[idx] == SX1302_PKT_SYNCWORD_BYTE_0) && (self->buffer[idx + 1] == SX1302_PKT_SYNCWORD_BYTE_1)) {
                ...
                SID_HAL_LOG_INFO("INFO: syncword not found at idx %d\n", idx);
                idx += 1;
            }
        }
        if (idx > self->buffer_size - 2) {
            SID_HAL_LOG_WARNING("WARNING: no syncword found, discard rx_buffer\n");
            return rx_buffer_del(self);
        }
        if (idx != 0) {
            SID_HAL_LOG_INFO("INFO: re-sync rx_buffer at idx %d\n", idx);
        ...
        while (idx < self->buffer_size) {
            if ((self->buffer[idx] != SX1302_PKT_SYNCWORD_BYTE_0) || (self->buffer[idx + 1] != SX1302_PKT_SYNCWORD_BYTE_1)) {
                SID_HAL_LOG_WARNING("WARNING: syncword not found at idx %d, discard the rx_buffer\n", idx);
                return rx_buffer_del(self);
            }
...
int rx_buffer_pop(rx_buffer_t * self, rx_packet_t * pkt) {
    ...
    if((self->buffer_index + pkt_num_bytes) > self->buffer_size) {
        SID_HAL_LOG_WARNING("WARNING: aborting truncated message (size=%u)\n", self->buffer_size);
    ...
    if (checksum_rcv != checksum_calc) {
        SID_HAL_LOG_WARNING("WARNING: checksum failed (got:0x%02X calc:0x%02X)\n", checksum_rcv, checksum_calc);
        return LGW_REG_WARNING;
    } else {
    ...
    if (pkt->modem_id > SX1302_FSK_MODEM_ID) {
        SID_HAL_LOG_ERROR("ERROR: modem_id is out of range - %u\n", pkt->modem_id);
        return LGW_REG_ERROR;
    } else {
        if (pkt->modem_id <= SX1302_LORA_STD_MODEM_ID) { /* LoRa modems */
            if (pkt->rx_channel_in > 9) {
                SID_HAL_LOG_ERROR("ERROR: channel is out of range - %u\n", pkt->rx_channel_in);
                return LGW_REG_ERROR;
            }
            if ((pkt->rx_rate_sf < 5) || (pkt->rx_rate_sf > 12)) {
                SID_HAL_LOG_ERROR("ERROR: SF is out of range - %u\n", pkt->rx_rate_sf);
                return LGW_REG_ERROR;
            }
        } else { /* FSK modem */
            /* TODO: not checked */
...
void rx_buffer_dump(uint16_t start_addr, uint16_t end_addr) {
    uint8_t *rx_buffer_debug = NULL;

    if ((rx_buffer_debug = (uint8_t *)malloc(4096 * sizeof(uint8_t))) == NULL) {
        SID_HAL_LOG_ERROR("rx_buffer_debug malloc fail\n");
        goto ret;
    }
    SID_HAL_LOG_INFO("Dumping %u bytes, from 0x%X to 0x%X\n",
    ...
    SID_HAL_LOG_HEXDUMP_INFO(rx_buffer_debug, (end_addr - start_addr + 1));
