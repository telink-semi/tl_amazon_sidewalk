...
_attribute_ble_data_retention_ u8 ble_mac_public[6];

_attribute_ble_data_retention_ u32 subg_trim_addr = CFG_ADR_SIDEWALK_1M_TRIM_FLASH;
...
extern void ble_connect_cb(uint16_t connHandle, uint8_t* addr, uint8_t err);
extern void ble_disconnect_cb(uint16_t connHandle, uint8_t reason);
extern void ble_mtu_cb(uint16_t connHandle, uint16_t tx_mtu, uint16_t rx_mtu);
/**
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        ble_connect_cb(pConnEvt->connHandle, pConnEvt->peerAddr, BLE_SUCCESS);
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    ble_disconnect_cb(pDisConn->connHandle, pDisConn->reason );
...
int app_le_connection_update_complete_event_handle(u8 *p)
{
    ...
    if (pUpt->status == BLE_SUCCESS) {
        ble_adpter_set_last_conn_param(pUpt->connInterval, pUpt->connInterval,pUpt->connLatency,pUpt->supervisionTimeout) ;
    }
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
    ...
    case GAP_EVT_ATT_EXCHANGE_MTU:
    {
        ...
        tlkapi_printf(APP_LOG_EN, "[APP][EVT] GAP_EVT_ATT_EXCHANGE_MTU handle:%04X peer_MTU:%04X effective_MTU:%04X", pEvt->connHandle, pEvt->peer_MTU, pEvt->effective_MTU);
        ble_mtu_cb(pEvt->connHandle, pEvt->peer_MTU, pEvt->effective_MTU);
    } break;
...
void app_sid_flash_settting(void)
{
    void fal_flash_start_addr_set(u32 addr);
    uint32_t sid_mfg_set_start_addr(u32 addr);

    if (blc_flash_capacity == FLASH_SIZE_1M) {

        fal_flash_start_addr_set(CFG_ADR_SIDEWALK_1M_FLASH);
        sid_mfg_set_start_addr(CFG_ADR_SIDEWALK_MFG_1M_FLASH);
        subg_trim_addr = CFG_ADR_SIDEWALK_1M_TRIM_FLASH;
        tlkapi_printf(APP_LOG_EN, "app_sid_flash_settting 1M Flash,  mfg on 0x%x ,nv on 0x%x\r\n",CFG_ADR_SIDEWALK_MFG_1M_FLASH,CFG_ADR_SIDEWALK_1M_FLASH);
    }
    else if (blc_flash_capacity == FLASH_SIZE_2M) {

        fal_flash_start_addr_set(CFG_ADR_SIDEWALK_2M_FLASH);
        sid_mfg_set_start_addr(CFG_ADR_SIDEWALK_MFG_2M_FLASH);
        subg_trim_addr = CFG_ADR_SIDEWALK_2M_TRIM_FLASH;
        tlkapi_printf(APP_LOG_EN, "app_sid_flash_settting 2M Flash,  mfg on 0x%x ,nv on 0x%x\r\n",CFG_ADR_SIDEWALK_MFG_2M_FLASH,CFG_ADR_SIDEWALK_2M_FLASH);
   }
...
//    u8 mac_public[6];
    ...
    blc_initMacAddress(flash_sector_mac_address, ble_mac_public, mac_random_static);
    ...
    blc_ll_initStandby_module(ble_mac_public);
...
#  elif (MCU_CORE_TYPE == MCU_CORE_TL323X)
    blc_pm_setDeepsleepRetentionEarlyWakeupTiming(920); //for os
#  endif /* MCU_CORE_TYPE */
# else
...
# endif /* PM_DEEPSLEEP_RETENTION_ENABLE */
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] Telink-Sidewalk BLE init", 0, 0);
    tlkapi_printf(APP_LOG_EN, "[APP][INI] MAC:%02X:%02X:%02X:%02X:%02X:%02X\n", ble_mac_public[0], ble_mac_public[1], ble_mac_public[2],
            ble_mac_public[3], ble_mac_public[4], ble_mac_public[5]);
}
...
void app_process_power_management(void)
{
    ...
    if (tlkapi_debug_isBusy() || sid_timer_is_forbidden_sleep()) {
...
int blc_bt_enable(void)
{
    return 0;
    // user_init_normal();
}

int blc_bt_disable(void)
{
    return 0;
    //todo
}
