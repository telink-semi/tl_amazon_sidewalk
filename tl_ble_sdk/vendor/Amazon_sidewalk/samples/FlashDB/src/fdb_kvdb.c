...
int fdb_prefix_delete(fdb_kvdb_t db, const char *prefix)
{
...
//    #define MAX_DEL 32
//    const char *name_buf[MAX_DEL];
//     char name_len[MAX_DEL];
//    uint16_t    cnt = 0;
    ...
    while (fdb_kv_iterate(db, &it)) {
        kv = &it.curr_kv;
        if (strncmp(kv->name, prefix, prefix_len) == 0) {
            fdb_kv_del(db, kv->name);
            goto again;
//            if (cnt >= MAX_DEL) break;
//            name_buf[cnt] = kv->name;
//            name_len[cnt] = kv->name_len;
//            cnt++;
        }
    }
//    char name[FDB_KV_NAME_MAX+1];
//
//    for (uint16_t i = 0; i < cnt; ++i) {
////        strncpy(name,name_buf[i],name_len[i]);
//
//        fdb_err_t err = fdb_kv_del(db, name_buf[i]);
//        if (err != FDB_NO_ERR)
//            return err;
//    }
    return FDB_NO_ERR;
 ...
 * @return KV object when is not NULL
 */
int fdb_kv_get_obj_len(fdb_kvdb_t db, const char *key)
{
    bool find_ok = false;
    struct fdb_kv kv;
