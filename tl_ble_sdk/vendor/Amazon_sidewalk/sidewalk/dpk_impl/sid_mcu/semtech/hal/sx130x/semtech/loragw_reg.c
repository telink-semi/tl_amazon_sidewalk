...
int reg_w(uint8_t spi_mux_target, struct lgw_reg_s r, int32_t reg_value) {
    ...
    if ((r.leng == 8) && (r.offs == 0)) {
        ...
        SID_HAL_LOG_ERROR("ERROR: REGISTER SIZE AND OFFSET ARE NOT SUPPORTED\n");
...
int lgw_connect(const lgw_com_type_t com_type, const char * com_path) {
    ...
    if (com_path == NULL) {
        SID_HAL_LOG_ERROR("ERROR: COM PATH IS NOT SET\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR CONNECTING CONCENTRATOR\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR READING CHIP VERSION REGISTER\n");
        return LGW_REG_ERROR;
    }
    SID_HAL_LOG_INFO("Note: chip version is 0x%02X (v%u.%u)\n", u, (u >> 4) & 0x0F, u & 0x0F) ;
...
int lgw_disconnect(void) {
    ...
    if (com_stat == LGW_COM_SUCCESS) {
        ...
        SID_HAL_LOG_ERROR("ERROR: Failed to disconnect the concentrator\n");
        return LGW_REG_ERROR;
    }
}
...
int lgw_reg_w(uint16_t register_id, int32_t reg_value) {
    ...
    if (register_id >= LGW_TOTALREGS) {
        SID_HAL_LOG_ERROR("ERROR: REGISTER NUMBER OUT OF DEFINED RANGE\n");
        return LGW_REG_ERROR;
    }
    ...
    if (r.rdon == 1){
        SID_HAL_LOG_ERROR("ERROR: TRYING TO WRITE A READ-ONLY REGISTER\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: COM ERROR DURING REGISTER WRITE\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int lgw_reg_wb(uint16_t register_id, uint8_t *data, uint16_t size) {
    ...
    /* reject write to read-only registers */
    if (r.rdon == 1){
        SID_HAL_LOG_ERROR("ERROR: TRYING TO BURST WRITE A READ-ONLY REGISTER\n");
        return LGW_REG_ERROR;
    }
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: COM ERROR DURING REGISTER BURST WRITE\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
...
int lgw_reg_rb(uint16_t register_id, uint8_t *data, uint16_t size) {
    ...
    if (com_stat != LGW_COM_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: COM ERROR DURING REGISTER BURST READ\n");
        return LGW_REG_ERROR;
    } else {
        return LGW_REG_SUCCESS;
    }
