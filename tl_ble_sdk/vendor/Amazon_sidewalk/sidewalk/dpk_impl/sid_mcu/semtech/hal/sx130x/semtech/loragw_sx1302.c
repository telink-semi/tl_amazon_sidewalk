...
static rx_buffer_t *rx_buffer = NULL;
...
int calculate_freq_to_time_drift(uint32_t freq_hz, uint8_t bw, uint16_t * mant, uint8_t * exp) {
    ...
    if (bw_hz < 0) {
        SID_HAL_LOG_ERROR("ERROR: Unsupported bandwidth for frequency to time drift calculation\n");
...
int sx1302_init(const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (!rx_buffer) {
        if ((rx_buffer = (rx_buffer_t *)malloc(sizeof(struct rx_buffer_s))) == NULL) {
            SID_HAL_LOG_ERROR("ERROR: rx_buffer failed to malloc \n");
            return LGW_REG_ERROR;
        }
    }
    ...
    if (ftime_context->enable == true) {
        ...
        if (x != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: failed to get Chip Model ID\n");
            return LGW_REG_ERROR;
        }

        if (model_id != CHIP_MODEL_ID_SX1303) {
            SID_HAL_LOG_ERROR("ERROR: Fine Timestamping is not supported on this Chip Model ID 0x%02X\n", model_id);
            return LGW_REG_ERROR;
        }
    }
    x = timestamp_counter_mode(ftime_context->enable);
    if (x != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure timestamp counter mode\n");
        return LGW_REG_ERROR;
    }

    x = sx1302_config_gpio();
    if (x != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to configure sx1302 GPIOs\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_update(void) {
    ...
    if (val != 0) {
        SID_HAL_LOG_ERROR("ERROR: Parity error check failed on AGC firmware\n");
        return LGW_REG_ERROR;
    }
    lgw_reg_r(SX1302_REG_ARB_MCU_CTRL_PARITY_ERROR, &val);
    if (val != 0) {
        SID_HAL_LOG_ERROR("ERROR: Parity error check failed on ARB firmware\n");
        return LGW_REG_ERROR;
    }
#endif
...
int sx1302_radio_clock_select(uint8_t rf_chain) {
    ...
    {
        SID_HAL_LOG_ERROR("ERROR: invalid RF chain\n");
        return LGW_REG_ERROR;
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to select radio clock for radio_%u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_reset(uint8_t rf_chain, lgw_radio_type_t type) {
    ...
    if ((type != LGW_RADIO_TYPE_SX1255) && (type != LGW_RADIO_TYPE_SX1257) && (type != LGW_RADIO_TYPE_SX1250)) {
        SID_HAL_LOG_ERROR("ERROR: invalid radio type\n");
        return LGW_REG_ERROR;
    }
    ...
    /* Check if something went wrong */
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to reset the radios\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_set_mode(uint8_t rf_chain, lgw_radio_type_t type) {
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to set mode for radio %u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_calibrate(struct lgw_conf_rxrf_s * context_rf_chain, uint8_t clksrc, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (context_rf_chain[i].enable == true) {
            ...
            if (err != LGW_REG_SUCCESS) {
                SID_HAL_LOG_ERROR("ERROR: failed to set radio %d mode\n", i);
                return LGW_REG_ERROR;
            }
        }
    }
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to get select clock from radio %u\n", clksrc);
        return LGW_REG_ERROR;
    }
    ...
    //     if (err != LGW_REG_SUCCESS) {
    //         SID_HAL_LOG_ERROR("ERROR: Failed to load calibration fw\n");
    //         return LGW_REG_ERROR;
    //     }
...
int sx1302_lora_service_correlator_configure(struct lgw_conf_rxif_s * cfg) {
    ...
    switch (cfg->datarate) {
        ...
        case 12:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: Failed to configure LoRa service modem correlators\n");
            return LGW_REG_ERROR;
    }
...
int sx1302_lora_modem_configure(uint32_t radio_freq_hz) {
    ...
    if (calculate_freq_to_time_drift(radio_freq_hz, BW_125KHZ, &mantissa, &exponent) != 0) {
        SID_HAL_LOG_ERROR("ERROR: failed to calculate frequency to time drift for LoRa modem\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_lora_service_modem_configure(struct lgw_conf_rxif_s * cfg, uint32_t radio_freq_hz) {
        ...
        case DR_LORA_SF12:
            ...
            switch (cfg->bandwidth) {
                ...
                case BW_500KHZ:
                    ...
                    break;
                default:
                    SID_HAL_LOG_ERROR("ERROR: unsupported bandwidth %u for LoRa Service modem\n", cfg->bandwidth);
                    break;
            }
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: unsupported datarate %lu for LoRa Service modem\n", cfg->datarate);
            break;
    }
    ...
    if (calculate_freq_to_time_drift(radio_freq_hz, cfg->bandwidth, &mantissa, &exponent) != 0) {
        SID_HAL_LOG_ERROR("ERROR: failed to calculate frequency to time drift for LoRa service modem\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    int32_t val;
    int err = LGW_REG_SUCCESS;
    //TODO: is need to verify firmware?
    //malloc to prevent stack overflow MCU_FW will be 8k bytes.
    uint8_t *fw_check;
    if ((fw_check = (uint8_t*)malloc(MCU_FW_SIZE * sizeof(uint8_t))) == NULL) {
        SID_HAL_LOG_ERROR("ERROR: AGC fw_check malloc failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
    ...
    if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
        SID_HAL_LOG_ERROR("ERROR: AGC fw read/write check failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
    ...
    if (val != 0) {
        SID_HAL_LOG_ERROR("ERROR: Failed to load AGC fw: parity error check failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
...
int sx1302_agc_status(uint8_t* status) {
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to get AGC status\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_agc_mailbox_write(uint8_t mailbox, uint8_t value) {
    ...
    if (lgw_reg_w(reg, (int32_t)value) != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to write AGC mailbox\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_agc_start(uint8_t version, lgw_radio_type_t radio_type, uint8_t ana_gain, uint8_t dec_gain, bool full_duplex, bool lbt_enable) {
    ...
    if (val != version) {
        SID_HAL_LOG_ERROR("ERROR: wrong AGC fw version (%d)\n", val);
        return LGW_REG_ERROR;
    }
    ...
    if (radio_type != LGW_RADIO_TYPE_SX1250) {
        SID_HAL_LOG_ERROR("AGC: setting fdd_mode to %u\n", fdd_mode);
    ...
    sx1302_agc_mailbox_read(0, &val);
    if (val != ana_gain) {
        SID_HAL_LOG_ERROR("ERROR: Analog gain of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != dec_gain) {
        SID_HAL_LOG_ERROR("ERROR: Decimator gain of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != fdd_mode) {
        SID_HAL_LOG_ERROR("ERROR: FDD mode of Radio A has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    sx1302_agc_mailbox_read(0, &val);
    if (val != ana_gain) {
        SID_HAL_LOG_ERROR("ERROR: Analog gain of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    sx1302_agc_mailbox_read(1, &val);
    if (val != dec_gain) {
        SID_HAL_LOG_ERROR("ERROR: Decimator gain of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    sx1302_agc_mailbox_read(2, &val);
    if (val != fdd_mode) {
        SID_HAL_LOG_ERROR("ERROR: FDD mode of Radio B has not been set properly\n");
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.ana_min) {
        SID_HAL_LOG_ERROR("ERROR: wrong ana_min (w:%u r:%u)\n", agc_params.ana_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.ana_max) {
        SID_HAL_LOG_ERROR("ERROR: ana_max (w:%u r:%u)\n", agc_params.ana_max, val);
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.ana_thresh_l) {
        SID_HAL_LOG_ERROR("ERROR: wrong ana_thresh_l (w:%u r:%u)\n", agc_params.ana_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.ana_thresh_h) {
        SID_HAL_LOG_ERROR("ERROR: wrong ana_thresh_h (w:%u r:%u)\n", agc_params.ana_thresh_h, val);
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.dec_attn_min) {
        SID_HAL_LOG_ERROR("ERROR: wrong dec_attn_min (w:%u r:%u)\n", agc_params.dec_attn_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.dec_attn_max) {
        SID_HAL_LOG_ERROR("ERROR: wrong dec_attn_max (w:%u r:%u)\n", agc_params.dec_attn_max, val);
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.dec_thresh_l) {
        SID_HAL_LOG_ERROR("ERROR: wrong dec_thresh_l (w:%u r:%u)\n", agc_params.dec_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.dec_thresh_h1) {
        SID_HAL_LOG_ERROR("ERROR: wrong dec_thresh_h1 (w:%u r:%u)\n", agc_params.dec_thresh_h1, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(2, &val);
    if (val != agc_params.dec_thresh_h2) {
        SID_HAL_LOG_ERROR("ERROR: wrong dec_thresh_h2 (w:%u r:%u)\n", agc_params.dec_thresh_h2, val);
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.chan_attn_min) {
        SID_HAL_LOG_ERROR("ERROR: wrong chan_attn_min (w:%u r:%u)\n", agc_params.chan_attn_min, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.chan_attn_max) {
        SID_HAL_LOG_ERROR("ERROR: wrong chan_attn_max (w:%u r:%u)\n", agc_params.chan_attn_max, val);
        return LGW_REG_ERROR;
    }
    ...
    if (val != agc_params.chan_thresh_l) {
        SID_HAL_LOG_ERROR("ERROR: wrong chan_thresh_l (w:%u r:%u)\n", agc_params.chan_thresh_l, val);
        return LGW_REG_ERROR;
    }
    sx1302_agc_mailbox_read(1, &val);
    if (val != agc_params.chan_thresh_h) {
        SID_HAL_LOG_ERROR("ERROR: wrong chan_thresh_h (w:%u r:%u)\n", agc_params.chan_thresh_h, val);
        return LGW_REG_ERROR;
    }
    ...
    if (radio_type == LGW_RADIO_TYPE_SX1250) {
        ...
        if (val != agc_params.deviceSel) {
            SID_HAL_LOG_ERROR("ERROR: wrong deviceSel (w:%u r:%u)\n", agc_params.deviceSel, val);
            return LGW_REG_ERROR;
        }
        sx1302_agc_mailbox_read(1, &val);
        if (val != agc_params.hpMax) {
            SID_HAL_LOG_ERROR("ERROR: wrong hpMax (w:%u r:%u)\n", agc_params.hpMax, val);
            return LGW_REG_ERROR;
        }
        sx1302_agc_mailbox_read(2, &val);
        if (val != agc_params.paDutyCycle) {
            SID_HAL_LOG_ERROR("ERROR: wrong paDutyCycle (w:%u r:%u)\n", agc_params.paDutyCycle, val);
            return LGW_REG_ERROR;
        }
    ...
    if (val != pa_start_delay) {
        SID_HAL_LOG_ERROR("ERROR: wrong PA start delay (w:%u r:%u)\n", pa_start_delay, val);
        return LGW_REG_ERROR;
    }
    ...
    if ((bool)val != lbt_enable) {
        SID_HAL_LOG_ERROR("ERROR: wrong LBT configuration (w:%u r:%u)\n", lbt_enable, val);
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    uint8_t *fw_check;
    if ((fw_check = (uint8_t*)malloc(MCU_FW_SIZE * sizeof(uint8_t))) == NULL) {
        SID_HAL_LOG_ERROR("ERROR: ARB fw_check malloc failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
    ...
    if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
        SID_HAL_LOG_ERROR("ERROR: ARB fw read/write check failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
    ...
    if (val != 0) {
        SID_HAL_LOG_ERROR("ERROR: Failed to load ARB fw: parity error check failed\n");
        err = LGW_REG_ERROR;
        goto ret;
    }
...
int sx1302_arb_status(uint8_t* status) {
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to get ARB status\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_debug_read(uint8_t reg_id, uint8_t* value) {
    ...
    /* Check parameters */
    if (reg_id > 15) {
        SID_HAL_LOG_ERROR("ERROR: invalid ARB debug register ID\n");
        return LGW_REG_ERROR;
    }

    reg = SX1302_REG_ARB_MCU_ARB_DEBUG_STS_0_ARB_DEBUG_STS_0 + reg_id;
    if (lgw_reg_r(reg, &val) != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to read ARB debug register\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_debug_write(uint8_t reg_id, uint8_t value) {
    ...
    if (lgw_reg_w(reg, (int32_t)value) != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: failed to write ARB debug register ID\n");
        return LGW_REG_ERROR;
    }
...
uint8_t sx1302_arb_get_debug_stats_detect(uint8_t channel) {
    ...
    if (channel >= 8) {
        SID_HAL_LOG_ERROR("ERROR: wrong configuration, channel num must be < 8");
        return 0;
    }
...
int sx1302_arb_start(uint8_t version, const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (val != version) {
        SID_HAL_LOG_ERROR("ERROR: wrong ARB fw version (%d)\n", val);
        return LGW_REG_ERROR;
    }
    ...
    if (ftime_context->enable == false) {
        ...
        if (ftime_context->mode == LGW_FTIME_MODE_ALL_SF) {
            ...
            SID_HAL_LOG_ERROR("ERROR: fine timestamp mode is not supported (%d)\n", ftime_context->mode);
            return LGW_REG_ERROR;
        }
    }
...
int sx1302_fetch(uint8_t * nb_pkt) {
    ...
    if (rx_buffer && (rx_buffer->buffer_pkt_nb == 0)) {
        /* Initialize RX buffer */
        err = rx_buffer_new(rx_buffer);
        if (err != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: Failed to initialize RX buffer\n");
            return LGW_REG_ERROR;
        }
        ...
        if (err != LGW_REG_SUCCESS) {
            SID_HAL_LOG_ERROR("ERROR: Failed to fetch RX buffer\n");
            return LGW_REG_ERROR;
        }
    } else {
        //SID_HAL_LOG_INFO("Note: remaining %u packets in RX buffer, do not fetch sx1302 yet...\n", rx_buffer->buffer_pkt_nb);
    }
...
int sx1302_parse(lgw_context_t * context, struct lgw_pkt_rx_s * p) {
    ...
    */
    ...
    if (p->if_chain >= LGW_IF_CHAIN_NB) {
        SID_HAL_LOG_WARNING("WARNING: %u NOT A VALID IF_CHAIN NUMBER, ABORTING\n", p->if_chain);
        return LGW_REG_ERROR;
    }
    ...
    if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
            ...
            if (pkt.payload_crc_error) {
                ...
                if (p->size > 0) {
                    ...
                    if (payload_crc16_calc != pkt.rx_crc16_value) {
                        SID_HAL_LOG_ERROR("ERROR: Payload CRC16 check failed (got:0x%04X calc:0x%04X)\n", pkt.rx_crc16_value, payload_crc16_calc);
                        return LGW_REG_ERROR;
                    } else {
        ...
        switch (p->bandwidth) {
            ...
            default:
                p->freq_offset = 0;
                SID_HAL_LOG_ERROR("Invalid frequency offset\n");
                break;
        }
        ...
        SID_HAL_LOG_ERROR("ERROR: UNEXPECTED PACKET ORIGIN\n");
    ...
    switch (radio_type) {
        case LGW_RADIO_TYPE_SX1250:
            if (bandwidth == BW_125KHZ) {
                ...
                SID_HAL_LOG_ERROR("ERROR: bandwidth not supported\n");
                return LGW_REG_ERROR;
            }
            break;
        case LGW_RADIO_TYPE_SX1255:
        case LGW_RADIO_TYPE_SX1257:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: radio type not supported\n");
            return LGW_REG_ERROR;
    }
...
float sx1302_rssi_get_temperature_offset(struct lgw_rssi_tcomp_s * context, float temperature) {
    ...
    CHECK_NULL(context);
#if 0
    SID_HAL_LOG_INFO("INFO: RSSI temperature compensation:\n");
    SID_HAL_LOG_INFO("       coeff_a: %.3f\n", context->coeff_a);
    SID_HAL_LOG_INFO("       coeff_b: %.3f\n", context->coeff_b);
    SID_HAL_LOG_INFO("       coeff_c: %.3f\n", context->coeff_c);
    SID_HAL_LOG_INFO("       coeff_d: %.3f\n", context->coeff_d);
    SID_HAL_LOG_INFO("       coeff_e: %.3f\n", context->coeff_e);
#endif
...
uint8_t sx1302_tx_status(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to read TX STATUS\n");
    ...
    if (read_value == 0x80) {
        ...
        SID_HAL_LOG_ERROR("ERROR: UNKNOWN TX STATUS 0x%02lX\n", read_value);
        return TX_STATUS_UNKNOWN;
    }
}
...
int sx1302_tx_abort(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        SID_HAL_LOG_ERROR("ERROR: Failed to stop TX trigger\n");
        return err;
    }
    ...
    do {
        ...
        if (timeout_check(tm_start, 1000) != 0) {
            SID_HAL_LOG_ERROR("ERROR: %s: TIMEOUT on TX abort\n", __FUNCTION__);
            return LGW_REG_ERROR;
        }
...
int sx1302_send(lgw_radio_type_t radio_type, struct lgw_tx_gain_lut_s * tx_lut, bool lwan_public, struct lgw_conf_rxif_s * context_fsk, struct lgw_pkt_tx_s * pkt_data) {
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_FSK:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: modulation type not supported\n");
            return LGW_REG_ERROR;
    }
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_FSK:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: Modulation not supported\n");
            return LGW_REG_ERROR;
    }
    ...
    switch (pkt_data->modulation) {
        ...
        case MOD_LORA:
            ...
            /* Chirp filtering */
            chirp_lowpass = (pkt_data->datarate < 7) ? 6 : 7;
    ...
    switch (pkt_data->tx_mode) {
        ...
        case ON_GPS:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: TX mode not supported\n");
            return LGW_REG_ERROR;
    }
