...
int sx1302_cal_start(uint8_t version, struct lgw_conf_rxrf_s * rf_chain_cfg, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    if (val != version) {
        SID_HAL_LOG_ERROR("ERROR: wrong CAL fw version (%d)\n", val);
...
#if DEBUG_CAL == 1
    DEBUG_MSG("-------------------------------------------------------------------\n");
    DEBUG_MSG("Radio calibration completed:\n");
...
int sx125x_cal_rx_image(uint8_t rf_chain, uint32_t freq_hz, bool use_loopback, uint8_t radio_type, struct lgw_sx125x_cal_rx_result_s * res) {
    ...
    switch (radio_type) {
        ...
        case LGW_RADIO_TYPE_SX1257:
            ...
            break;
        default:
            SID_HAL_LOG_ERROR("ERROR: UNEXPECTED VALUE %d FOR RADIO TYPE\n", radio_type);
            return LGW_HAL_ERROR;
    }
    ...
    if ((rx_pll_locked == 0) || (tx_pll_locked == 0)) {
        SID_HAL_LOG_ERROR("ERROR: PLL failed to lock\n");
        return LGW_HAL_ERROR;
    }
...
#if DEBUG_CAL == 1
    DEBUG_MSG("TX_SIG values returned by signal analyzer:");
    for (i = 0; i < 40; i++) {
        if (i%5 == 0) {
            DEBUG_MSG("\n");
        }
