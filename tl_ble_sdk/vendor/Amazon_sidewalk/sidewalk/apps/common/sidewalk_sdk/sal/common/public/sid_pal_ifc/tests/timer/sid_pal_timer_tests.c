 ...
 *
 * If necessary a platform can override this value, however if this is
 * required then consider whether the platform interrupt latency could also be
 * optimised.
 */
#ifndef TIMER_PERIODIC_ALLOWED_OVERHEAD_US
...
#define CALC_INSTANCE_FROM_TMR_PTR(tmr_ptr) \
    (((uintptr_t)(tmr_ptr) - (uintptr_t)test_timers) / sizeof(sid_pal_timer_t))
...
{
    RUN_TEST_CASE(TIMER, test_timer_init)
    RUN_TEST_CASE(TIMER, test_canceling_timer_fails_given_no_timer_arg)
    RUN_TEST_CASE(TIMER, test_timer_is_armed_fails_given_no_timer_arg)
    RUN_TEST_CASE(TIMER, test_unarmed_timer_can_be_canceled)
    RUN_TEST_CASE(TIMER, test_timeout_value_can_be_infinite)
    RUN_TEST_CASE(TIMER, test_arming_timer_fails_given_no_timer_arg)
    RUN_TEST_CASE(TIMER, test_arming_timer_fails_given_no_deadline)
    RUN_TEST_CASE(TIMER, test_one_shot_timers_fire_after_arming)
    // the next test case must run immediately after test_one_shot_timers_fire_after_arming
    RUN_TEST_CASE(TIMER, test_one_shot_timers_dont_repeat)
    RUN_TEST_CASE(TIMER, test_timer_can_be_canceled_after_arming)
    RUN_TEST_CASE(TIMER, test_periodic_timer_is_precise)
    RUN_TEST_CASE(TIMER, test_casual_timer_snaps)
    RUN_TEST_CASE(TIMER, test_casual_timer_doesnt_snap)
    RUN_TEST_CASE(TIMER, test_timer_deinit)
}
...
static void test_timer_cb_common(void *arg, sid_pal_timer_t *originator)
{
    ...
    if (expiry_event_q_write < LENGTH_OF_EXPIRY_QUEUE) {
        ...
        if (sid_time_is_zero(&timer_arg->next_timeout)) {
            ...
            if(sid_time_gt(&now, &timer_arg->next_timeout)) {
                sid_time_delta(&expiry_events[expiry_event_q_write].delay, &now, &timer_arg->next_timeout);
            } else {
                // The timer expired before the scheduled time. The test function will check that the drift is no more than TIMER_PERIODIC_ALLOWED_OVERHEAD_US
                sid_time_delta(&expiry_events[expiry_event_q_write].delay, &timer_arg->next_timeout, &now);
            }
...
TEST(TIMER, test_timeout_value_can_be_infinite)
{
    struct sid_timespec when = SID_TIME_INFINITY;
    const sid_error_t ret = sid_pal_timer_arm(&test_timers[0], SID_PAL_TIMER_PRIO_CLASS_PRECISE, &when, NULL);
    TEST_ASSERT_EQUAL(SID_ERROR_NONE, ret);
    const sid_error_t cancel_ret = sid_pal_timer_cancel(&test_timers[0]);
    TEST_ASSERT_EQUAL(SID_ERROR_NONE, cancel_ret);
}

TEST(TIMER, test_arming_timer_fails_given_no_timer_arg)
{
...
TEST(TIMER, test_arming_timer_fails_given_no_deadline)
{
                                      ...
                                      const struct sid_timespec *period)
{
    ...
    const sid_error_t ret = sid_clock_now(SID_CLOCK_SOURCE_UPTIME, &first_shot, NULL);
...
TEST(TIMER, test_one_shot_timers_fire_after_arming)
{
    uint32_t tmr_inst;
    ...
    for (polling_time_ms = 0; polling_time_ms < TOTAL_ONE_SHOT_TEST_TIME_MS; ) {
        sid_pal_scheduler_delay_ms(POLLING_TIMER_GRANULARITY_MS);
...
TEST(TIMER, test_one_shot_timers_dont_repeat)
{
    uint32_t tmr_inst;
...
TEST(TIMER, test_timer_can_be_canceled_after_arming)
{
...
TEST(TIMER, test_casual_timer_snaps)
{
    ...
    while(expiry_event_q_write < expected_events) {
        sid_pal_scheduler_delay_ms(TIMER_CASUAL_PERIOD_MS);
    }
...
#else
    /* This test can only be executed on a platform that properly implements
     * casual timers. Some platforms currently just deprioritise the timers
     * if there are multiple expiring at the same time, without adding the
     * casual nature that would allow timers to expire at a varying time.
     */
    TEST_IGNORE_MESSAGE("Skipping, unsupported on this platform.");
#endif
}

TEST(TIMER, test_casual_timer_doesnt_snap)
{
#ifdef SID_PAL_CASUAL_TIMER_TESTS
    struct sid_timespec offset, period;
    sid_error_t ret;
...
void sid_pal_timer_tests_run(void)
{
