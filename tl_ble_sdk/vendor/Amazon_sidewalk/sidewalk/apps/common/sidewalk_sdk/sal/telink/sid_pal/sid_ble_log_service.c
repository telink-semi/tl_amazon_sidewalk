...
#include <sid_ble_adapter_callbacks.h>
 ...
 void log_srv_notif_changed(uint16_t conn_handle, uint16_t handle, uint16_t value);
 int log_srv_on_write(uint16_t conn_handle, uint16_t handle, const void *buf, uint16_t len, uint16_t offset);
...
void log_srv_notif_changed(uint16_t conn_handle, uint16_t handle, uint16_t value)
{
    ...
    TL_LOG_D("Notification for LOGGING_SERVICE is %s.", notif_enabled ? "enabled" : "disabled");
...
int log_srv_on_write(uint16_t conn_handle, uint16_t handle, const void *buf, uint16_t len, uint16_t offset)
{
    ...
    TL_LOG_D("Data received for LOGGING_SERVICE [len=%d].", len);
