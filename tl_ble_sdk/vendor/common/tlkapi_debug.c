...
#else
_attribute_ram_ble_data_retention_ u8 print_fifo_b[TLKAPI_DEBUG_FIFO_SIZE * TLKAPI_DEBUG_FIFO_NUM];
#endif
...
#if TLKAPI_RTT_PRINT
__attribute__((weak)) __attribute__((used)) int _write(int fd, const unsigned char *buf, int size)
#else
...
#endif
{
    ...
    if (tlkDbgCtl.dbg_chn == TLKAPI_DEBUG_CHANNEL_UDB) {
        ...
        *pd++ = 0;
        ...
        unsigned int temp_len = size ;
        if(temp_len > tlkapi_print_fifo->size -4)
            temp_len = tlkapi_print_fifo->size -4;
        memcpy((char *)(pd + 4), buf, temp_len);
        int len = temp_len;
